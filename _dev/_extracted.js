...
const BOOST_THRUST = 0.42;  // higher thrust while Shift held — punchy acceleration
...
const BG_BASE  = 'space_parallax_backgrounds_v1/assets/';
...
function loadBgSet(name) {
  ...
  const defs = [
    ...
    { file: `flat/${name}_FLAT.png`,     speed: 0.05 },

    { file: `layers/${name}_L1_far.png`, speed: 0.20 },

    { file: `layers/${name}_L2_mid.png`, speed: 0.50 },

    { file: `layers/${name}_L3_near.png`,speed: 1.00 },
...
async function loadAll() {
  ...
  const bgDefs = [
    ...
    { file: `flat/${bgName}_FLAT.png`,     speed: 0.05 },

    { file: `layers/${bgName}_L1_far.png`, speed: 0.20 },

    { file: `layers/${bgName}_L2_mid.png`, speed: 0.50 },

    { file: `layers/${bgName}_L3_near.png`,speed: 1.00 },
...
const HUD_FONT  = '10px Courier New';
...
function hLine(label, value, x, y) {
  ...
  // Unified label + value row — same font, same size, everywhere
...
function drawHUD(speed) {
  ...
  // Dark semi-transparent panel behind all HUD text

  // Dark opaque panel — must be readable over any background
  ...
  hLine('SPD', speed.toFixed(2),                                                 LEFT, TOP + 10);

  hLine('DIR', ship.dir.replace('-', ' ').toUpperCase(),                         LEFT, TOP + 23);

  hLine('POS', Math.floor(ship.worldX) + ' / ' + Math.floor(ship.worldY),       LEFT, TOP + 36);



  // ── THRUST bar ── y = 65

  const bx = LEFT, by = TOP + 55;
  ...
  const fx = LEFT, fy = 115;

  const segCount = 10, segW = 13, segH = 10, segGap = 2;
  ...
  for (let i = 0; i < segCount; i++) {
    ...
    if (!lit) {
      ...
      fill = (fuelPct < 0.1 && Math.floor(Date.now() / 300) % 2 === 0)
  ...
  ctx.fillText(
    ...
    ship.fuel.toFixed(1) + ' GAL  \u00b7  ' + ship.mpg + ' MPG',
  ...
  ctx.fillText('ORE', LEFT, oreY);

  ctx.fillStyle = ship.ore > 0 ? '#FFD700' : '#FFD70035';

  ctx.fillText(ship.ore + ' NEBULITE', LEFT + 30, oreY);
  ...
  // Pilot count (multiplayer only)
  if (typeof multiMode !== "undefined" && multiMode) {
    const pilotY = armY + 22;
  ...
  // Ore bar (cap display at 20 ore)

  const oreBarX = LEFT, oreBarY = oreY + 7;

  const oreBarW = 160, oreBarH = 4;
  ...
  if (ship.ore > 0) {
    ...
    const og = ctx.createLinearGradient(oreBarX, 0, oreBarX + oreBarW, 0);

    og.addColorStop(0, '#a07800');

    og.addColorStop(1, '#FFD700');

    ctx.fillStyle = og;

    ctx.fillRect(oreBarX, oreBarY, Math.min(ship.ore / 20, 1) * oreBarW, oreBarH);
  ...
  ctx.strokeRect(oreBarX-1, oreBarY-1, oreBarW+2, oreBarH+2);
  ...
  ctx.fillText('[C] REFINE', LEFT, oreBarY + 18);

  ctx.fillStyle = canCraft ? HUD_COLOR : '#2a3040';

  ctx.fillText(ORE_PER_FUEL + ' ORE → +' + FUEL_PER_CRAFT.toFixed(1) + ' FUEL', LEFT + 72, oreBarY + 18);



  // Mine proximity hint

  const mineHintY = oreBarY + 32;

  ctx.fillStyle = mineTarget ? '#4FC3C3' : '#2a3a4a';

  ctx.fillText('[E] MINE', LEFT, mineHintY);

  if (mineTarget) {

    ctx.fillStyle = '#4FC3C380';

    ctx.fillText(Math.round(mineDist) + 'px', LEFT + 60, mineHintY);
  ...
  const cx2 = canvas.width - 48, cy2 = 112;
  ...
  if (dirIdx >= 0) {
    ...
    ctx.lineTo(cx2 + Math.cos(a) * (r - 4), cy2 + Math.sin(a) * (r - 4));
    ...
    ctx.arc(cx2 + Math.cos(a) * (r-4), cy2 + Math.sin(a) * (r-4), 2.5, 0, Math.PI*2);
  ...
  // Map name flash after switching
  ...
  if (bgFlash && bgFlash.timer > 0) {
    ...
    ctx.fillText(
      ...
      'MAP: ' + bgFlash.name.replace('_', ' ').toUpperCase(),
...
// Full agent-readable game state panel. Shows every parameter relevant to

// understanding or extending this prototype. Includes last 30 input events.
