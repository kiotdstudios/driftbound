...
const ASTEROID_TYPES = [
  ...
  { id: 'lg_brown',  hp: 5, oreMin: 2, oreMax: 3, w: 40, h: 39, lootType: 'mineral',     lootChance: 0.02, scale: 4 }, // 160×156 — materially larger than ship (136px)
  { id: 'lg_planet', hp: 6, oreMin: 3, oreMax: 5, w: 42, h: 39, lootType: 'armalcolite', lootChance: 0.35, scale: 4 }, // 168×156 — approx 168px per spec
];
...
function loadBgSet(name) {
  ...
  Promise.all(jobs).then(layers => {
    ...
    layers.forEach((layer, i) => {
      ...
      const idx = [0.05, 0.20, 0.50, 1.00].indexOf(layer.speed);
...
// ═══════════════════════════════════════════════════════════════════════════
// _bgImgs[0]=FLAT(static base), [1]=L1_far(par 0.04), [2]=L2_mid(par 0.13), [3]=L3_near(par 0.28)
const _bgImgs = [null, null, null, null];
const _BG_PAR = [0, 0.04, 0.13, 0.28];
const _BG_ALP = [1.0, 0.65, 0.75, 0.85];
...
async function loadAll() {
  ...
  const _bgFiles = ['vapor_02_FLAT','vapor_02_L1_far','vapor_02_L2_mid','vapor_02_L3_near'];
  _bgFiles.forEach((name, i) => {
    bgLoad(`vapor_bg/${name}.png`, img => { _bgImgs[i] = img; });
  });
...
// Observers will only notice movement after watching for several seconds.
const NEB_LAYERS = [
  { vx:  0.0, vy:  0.0 },  // [0] FLAT base — completely static anchor
  { vx:  0.7, vy:  0.22 }, // [1] far layer  — barely imperceptible rightward drift
  { vx: -0.4, vy:  0.55 }, // [2] mid layer  — slow opposing drift gives depth
  { vx:  1.2, vy: -0.18 }, // [3] near layer — slightly faster, different angle
];
...
function updateMining() {
  ...
  // [E] — interior entry (near attached pod) or mine
...
function drawWorldPods(cx, cy) {
  ...
  for (const pod of worldPods) {
    ...
    // ── Draw procedural modular pod ──
    ctx.save();
    ctx.translate(sx, sby);

    // Glow
    const glowR = inRange ? 22 + pulse * 8 : 10;
    const glow = ctx.createRadialGradient(0, 0, 0, 0, 0, glowR);
    glow.addColorStop(0, col + (inRange ? 'aa' : '44'));
    ...
    const panelW = 28, panelH = 7;
    // Left panel
    ctx.fillStyle = '#1a2535';
    ctx.strokeStyle = '#38bdf844';
    ctx.lineWidth = 0.7;
    ctx.fillRect(-R - panelW, -panelH/2, panelW, panelH);
    ctx.strokeRect(-R - panelW, -panelH/2, panelW, panelH);
    // Panel cells
    for (let k = 0; k < 4; k++) {
      ctx.strokeStyle = '#38bdf833';
      ctx.strokeRect(-R - panelW + k*7 + 1, -panelH/2 + 1, 5, panelH - 2);
    }
    // Right panel
    ctx.fillStyle = '#1a2535';
    ctx.strokeStyle = '#38bdf844';
    ctx.fillRect(R, -panelH/2, panelW, panelH);
    ctx.strokeRect(R, -panelH/2, panelW, panelH);
    for (let k = 0; k < 4; k++) {
      ctx.strokeStyle = '#38bdf833';
      ctx.strokeRect(R + k*7 + 1, -panelH/2 + 1, 5, panelH - 2);
    }

    // Status beacon blink (top-right of body)
    const blinkOn = Math.floor(t * 2) % 2 === 0;
    ctx.shadowColor = blinkOn ? '#ff4444' : 'transparent';
    ctx.shadowBlur  = blinkOn ? 8 : 0;
    ctx.fillStyle   = blinkOn ? '#ff4444' : '#331111';
    ctx.beginPath(); ctx.arc(R * 0.55, -R * 0.55, 2.5, 0, Math.PI*2); ctx.fill();
    ctx.shadowBlur = 0;
...
function drawBG(camX, camY) {
  ...
  if (_bgImgs[0]) _drawNebulaLayer(_bgImgs[0], 0, 1.0);
...
function drawHUD(speed) {
  ...
  const R_H    = 18;   // standard row height
  const B_H    =  4;   // bar fill height
  const B_OFF  =  4;   // bar top from row top (so bar is vertically centered)
  const TXT    = 13;   // text baseline offset from row top (consistent across all rows)
  const DIV_A  =  4;   // space above divider line
  const DIV_B  =  6;   // space below divider line  (total divider block = DIV_A + 1 + DIV_B = 11)
  const SEC_B  =  4;   // space below section label
  ...
  // ── Content layout ──
  const L  = PX + PAD_X;         // left text start x
  const R  = PX + PW - PAD_X;    // right-aligned values end x
  const LV = L + 48;             // label column right edge / bar left edge
  const BW = R - LV;             // bar width
...
function update() {
  ...
  if (interiorMode) { requestAnimationFrame(loop); return; }
