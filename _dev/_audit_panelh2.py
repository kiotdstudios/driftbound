...
# BASE from patched code: 24+66+26 + 24+20+20+20 + 24+14 + 24+44 + 34
...
for scenario in [
    ...
    # simulate y walk (same as drawHUD)
    y=30
    y+=LINE+2; y+=LINE; y+=LINE; y+=LINE; y+=LINE+4  # nav sbar + 3 kv + gap
    y+=LINE+2; y+=4; y+=16; y+=4; y+=16; y+=4; y+=16  # ship sbar + thrust + hull + fuel
    if fw: y+=LINE  # fuel warning
    y+=LINE+2; y+=14  # cargo sbar + bar
    y+=rc*LINE  # resource rows
    if eh==1: y+=LINE  # empty hold
    if mc: y+=LINE+2; y+=mc*LINE  # modules sbar + rows
    y+=LINE+2; y+=LINE; y+=LINE  # actions sbar + 2 rows
    if nr: y+=LINE+2; y+=LINE  # network
    y+=14  # bottom padding
    needed=y-10
    match = 'OK' if panelH==needed else f'OFF by {panelH-needed}'
    print(f'  [{label}] panelH={panelH} needed={needed} -> {match}')
