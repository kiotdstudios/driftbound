...
// Minimap (World/UI ownership — see OWNERSHIP.md). Owns minimap presentation
...
// Minimap extracted to src/render/minimap.js (World/UI ownership, see
// OWNERSHIP.md) — behavior-preserving move. See createMinimap() /
// minimap.render() call site in loop() below.
...
window.__DB = {
  ...
  // ── Test-harness-only hooks (added for _dev/*.mjs migration off the legacy
  //    monolith globals; read/write pass-throughs to module state, no gameplay
  //    behavior change — see TEST HARNESS MIGRATION checkpoint). ──
  get frameCount(){ return frameCount; },
  get FUEL_CAPACITY(){ return FUEL_CAPACITY; },
  get BOOST_MAX(){ return BOOST_MAX; },
  get SHIP_MAX_HP(){ return SHIP_MAX_HP; },
  get MINE_RANGE(){ return MINE_RANGE; },
  get DEV_MODE(){ return DEV_MODE; },
  get DEV_COMMANDS(){ return DEV_COMMANDS; },
  get ASTEROID_TYPES(){ return ASTEROID_TYPES; },
  get POD_SPRITE_BASE(){ return POD_SPRITE_BASE; },
  get DOOR_COL(){ return DOOR_COL; },
  get DOOR_ROW(){ return DOOR_ROW; },
  get thrusting(){ return _thrusting; },
  get boosting(){ return _boosting; },
  get mineTarget(){ return mineTarget; },
  set mineTarget(v){ mineTarget = v; },
  get mineDist(){ return mineDist; },
  set mineDist(v){ mineDist = v; },
  get interiorFadeDir(){ return interiorFadeDir; },
  set interiorFadeDir(v){ interiorFadeDir = v; },
  get interiorFade(){ return interiorFade; },
  set interiorFade(v){ interiorFade = v; },
  get interiorPodIdx(){ return interiorPodIdx; },
  set interiorPodIdx(v){ interiorPodIdx = v; },
  get podSecured(){ return podSecured; },
  set podSecured(v){ podSecured = v; },
  get iPlayerX(){ return iPlayerX; },
  set iPlayerX(v){ iPlayerX = v; },
  get iPlayerY(){ return iPlayerY; },
  set iPlayerY(v){ iPlayerY = v; },
  get diagMode(){ return diagMode; },
